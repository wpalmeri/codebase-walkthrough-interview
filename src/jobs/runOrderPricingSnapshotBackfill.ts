...
import { runOrderPricingSnapshotBackfill } from "./orderPricingSnapshotBackfill";

const EnvironmentSchema = z.object({
  BACKFILL_BATCH_SIZE: z.coerce.number().int().min(1).max(10_000).default(100),
  /** Preview is the operational default; opt in to writes explicitly. */
...
async function main(): Promise<void> {
  ...
  const result = await runOrderPricingSnapshotBackfill({
