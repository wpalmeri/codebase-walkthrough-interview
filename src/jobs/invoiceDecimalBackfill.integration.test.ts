...
void test(
  "invoice Decimal backfill is dry-run safe, checkpointed, and restart-idempotent",
  { timeout: 30_000 },
  async () => {
    const temporaryDirectory = await mkdtemp(join(tmpdir(), "meridian-invoice-backfill-"));
    ...
    try {
      ...
      execFileSync(
        process.execPath,
        [join(process.cwd(), "src/jobs/invoiceDecimalBackfill.integration.scenario.ts")],
