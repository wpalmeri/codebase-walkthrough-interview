...
void test(
  "catalog APIs isolate tenant data and enforce catalog roles",
  { timeout: 30_000 },
  async () => {
    const temporaryDirectory = await mkdtemp(join(tmpdir(), "meridian-catalog-tenancy-"));
    ...
    try {
      ...
      execFileSync(
        process.execPath,
        [join(process.cwd(), "src/views/catalogTenantScope.integration.scenario.ts")],
