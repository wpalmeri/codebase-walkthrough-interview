...
import { LEGACY_DEFAULT_TENANT_ID } from "../tenancy/constants";
import { PrincipalSchema, TenantIdSchema, type Principal } from "./principal";

const BearerTokenSchema = z.string().min(1).max(512).regex(/^mrd_[A-Za-z0-9]{8,32}_[A-Za-z0-9-]{32,128}$/u);
const TenantApiKeyTokenSchema = z
  .string()
  .regex(/^mrd_(?<prefix>[A-Za-z0-9]{8,32})_(?<secret>[A-Za-z0-9-]{32,128})$/u);
...
export interface TenantApiKeyReader {
  findByPrefix(keyPrefix: string): Promise<TenantApiKeyRecord | null>;
}

export interface TenantPrincipalResolver {
...
function parsedKeyPrefix(token: string): string | null {
  const match = TenantApiKeyTokenSchema.safeParse(token);
  if (!match.success) return null;
  const prefix = /^mrd_(?<prefix>[A-Za-z0-9]{8,32})_/u.exec(match.data)?.groups?.prefix;
...
/** Resolves a strict opaque tenant key without retaining or logging its secret. */
export function createTenantPrincipalResolver(
  reader: TenantApiKeyReader,
  pepper: string
  ...
  return {
    async resolve(token) {
      ...
      return PrincipalSchema.parse({
        ...
        subjectId: `tenant:${record.tenantId}`,
...
export function createPrismaTenantPrincipalResolver(pepper: string): TenantPrincipalResolver {
  return createTenantPrincipalResolver(
    {
      async findByPrefix(keyPrefix) {
        return prisma.tenantApiKey.findUnique({
          where: { keyPrefix },
          select: { id: true, tenantId: true, role: true, keyHash: true, revokedAt: true },
        });
      },
    },
    pepper
...
export type AuthenticationOptions = {
  ...
  readonly principalResolver?: TenantPrincipalResolver;
};

function legacyPrincipal(tenantId: string): Principal {
  return PrincipalSchema.parse({
    tenantId,
...
/**
 * Authenticates credentials at the edge and writes a server-derived principal.
 * X-Tenant-ID is deliberately ignored: tenancy comes only from a verified key.
 */
export function createAuthenticationMiddleware(options: AuthenticationOptions): RequestHandler {
  const legacyTenantId = TenantIdSchema.parse(options.legacyTenantId ?? LEGACY_DEFAULT_TENANT_ID);
  return (request, response, next) => {
    ...
    if (options.legacyApiKey.length > 0 && tokensMatch(token, options.legacyApiKey)) {
      request.principal = legacyPrincipal(legacyTenantId);
      next();
      return;
    }
